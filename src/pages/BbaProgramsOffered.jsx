...
const WHY_CHOOSE_BBA_POINTS = [
  "Industry-Exposed Faculty",
  "Outcome-Based, Industry-Focused Curriculum",
  "Business Analytics & Digital Skills",
  "Entrepreneurship & Startup Programs",
  "Live Projects & Internships",
  "Group Discussions & Mock Interviews",
  "100% Internship Opportunities — UG & PG",
  "Strong Industry & Alumni Network"
];

export function BbaProgramsOffered() {
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);

  const specs = BBA_SPECIALIZATIONS.map(id => ({
    id,
    ...PROGRAMS_DATA[id]
  }));
    ...
    <main className={s.page}>
      <div className={s.container}>
        ...
        <header className={s.pageHeader}>
          <span className={s.pageSubtitle}>Undergraduate Programs</span>
          <h1 className={s.pageTitle}>BBA Specializations</h1>
        </header>

        {/* Introduction */}
        <section className={s.card} style={{ textAlign: "center", marginBottom: "48px" }}>
          <p className={s.listText} style={{ fontSize: "16px", maxWidth: "75ch", marginInline: "auto" }}>
            At MANSON School of Business, the BBA experience goes beyond classroom learning by combining industry certifications, practical exposure, internships, leadership development, and placement-focused training. Explore our 8 future-ready BBA pathways below.
          </p>
        </section>

        {/* Why Choose BBA at Manson Section */}
        <section className={s.card} style={{ border: "1px solid rgba(201, 162, 39, 0.25)", background: "#faf8f4", marginBottom: "56px" }}>
          <div style={{ textAlign: "center", marginBottom: "32px" }}>
            ...
            <h2 className={s.sectionHeader} style={{ fontSize: "26px", display: "block", marginTop: "8px" }}>
              Why Choose BBA at Manson School of Business
            </h2>
          </div>
          <div style={{
            display: "grid",
            ...
            {WHY_CHOOSE_BBA_POINTS.map((point, index) => (
              <div key={index} style={{
                display: "flex",
                alignItems: "center",
                gap: "12px",
        ...
        <section className={s.grid2} style={{ marginBottom: "64px" }}>
          {specs.map((program) => (
            ...
            >
              <div>
                <span className={s.badgeGreen} style={{ marginBottom: "12px" }}>BBA Specialization</span>
